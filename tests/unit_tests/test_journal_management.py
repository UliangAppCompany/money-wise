...
from account_management.models import Journal
...
class TestJournalModel: 
    ...
    def test_journal_create_double_entry(self, cursor, journal): 
        cursor.execute("select note from account_management_entry " 
                "where journal_id = %s", 
                [journal.id])
        result = cursor.fetchall() 
    ...
    def test_that_transaction_database_is_updated_with_double_entry(self, cursor, journal):
        ...
        assert result == [
                (100, Decimal('1050.00'), Decimal('0.00'), "from 300-Revenue account"), 
                (300, Decimal('0.00'), Decimal('1050.00'), "to 100-Cash account")]
    ...
    def execute_sql(self, cursor, stmt): 
        cursor.execute(stmt) 
        return cursor.fetchall()
    
    # @override_settings(DEBUG=True)
    def test_that_post_endpoint_responds(self,url,payload,stmt,db_expected, client):
        response = self.post_response(client, url, payload)
        assert response.status_code == 200

    def test_that_unauthenticated_user_cannot_create_journals(self,url,payload,stmt,db_expected, client): 
        client.logout()
        response = self.post_response(client, url, payload)
        assert response.status_code == 401

    def test_that_post_endpoint_adds_resource_to_db(self,url,payload,stmt,db_expected, client, cursor):
        self.post_response(client, url, payload) 
        result = self.execute_sql(cursor, stmt)

        assert result == db_expected


@pytest.mark.usefixtures('add_accounts_to_ledger', 'init_john', 'login_john')
@pytest.mark.parametrize('url,payload,stmt,db_expected', [
    ('/api/account-management/journal', {
        'number': 1, 
        'name': 'Company A Journal', 
        'description': 'description' 
    }, 
    """select name from account_management_journal where 
